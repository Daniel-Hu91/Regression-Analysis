...
selected_year = int(input("Enter the year to analyze [2018]: ").strip() or 2018)
...
independent_df = independent_df[independent_df["Date"].dt.year == selected_year]
dependent_df = dependent_df[dependent_df["Date"].dt.year == selected_year]
...
if df.empty:
    print("No overlapping data found for the selected year.")
    exit()
...
print(f"\nInitial correlation between {independent_label} and {dependent_label}: {corr:.4f}")
...
plt.title(f"{dependent_label} vs {independent_label} ({selected_year}, Original)")
...
plt.title(f"{dependent_label} vs {independent_label} ({selected_year}, Cleaned)")
plt.xlabel(independent_label)
plt.ylabel(dependent_label)
plt.grid(True)
plt.tight_layout()
...
plt.title(f"Residuals Before Filtering ({selected_year})")
plt.xlabel(independent_label)
plt.ylabel("Residual")
plt.legend()
plt.grid(True)
...
plt.title(f"Actual vs Fitted {dependent_label} ({selected_year}, Original)")
plt.xlabel("Date")
plt.ylabel(dependent_label)
plt.legend()
plt.grid(True)
...
plt.title(f"Actual vs Fitted {dependent_label} ({selected_year}, Cleaned)")
plt.xlabel("Date")
plt.ylabel(dependent_label)
plt.legend()
plt.grid(True)
...
plt.axhline(0, color="red", linestyle="--")
plt.title(f"Residual Plot After Filtering ({selected_year})")
...
# ---------------------------
# PLOT 7: BOTH SERIES OVER TIME (CLEANED)
# ---------------------------
...
plt.title(f"{dependent_label} and {independent_label} Over Time ({selected_year}, Cleaned)")
fig.tight_layout()
